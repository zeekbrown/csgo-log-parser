...
public static class PlayerThrewWeaponEvent
{
    private const string stamp = @"(\d\d/\d\d/\d\d\d\d - \d\d:\d\d:\d\d)";
    private const string player = @"""([^""]*)""";
    private const string weapon = @"(\w+) \[[^]]*\]";
    private static readonly Regex regex = new($"^L {stamp}: {player} threw {weapon}(?: flashbang entindex \\d+\\))?$");
